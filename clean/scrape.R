...
scrape <- function (outcome) {
  ...
  for (state in unique(state_fips_master$state)[!is.na(unique(state_fips_master$state))]) {
    for (year in 2011:2024) {
      ...
      url <- paste0("https://www.countyhealthrankings.org/health-data/health-factors/health-behaviors/sexual-activity/", 
                    outcome, 
                    "?year=",
                    year,
                    "&state=",
      ...
      # Save table as csv
      file_name <- paste0("data/", outcome, "/", state, "_", year, ".csv")
...
scrape("sexually-transmitted-infections")
scrape("teen-births")
